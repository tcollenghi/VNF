...
function LoadData() {
    $.ajax({
        ...
        cache: false,
        type: 'GET',
        url: url + "J1B1N_CFOP_Parceiro/GetData",
        dataType: 'json',
        data: {
        },
        error: function (erro) {
...
function Novo() {
    ...
    $("#txtTipoParceiro").val('');
    $("#hdfTipoParceiro").val('');
    $("#txtTipoParceiro").attr('disabled', false);
}

function Atualizar() {
    window.location = url + 'J1B1N_CFOP_Parceiro/Index';
}

function SalvarCFOP() {
    txtCFOP = $("#txtCFOP").val();
    txtTipoParceiro = $("#txtTipoParceiro").val();
    txtTipoParceiroAntigo = $("#hdfTipoParceiro").val();
    ...
    if (txtTipoParceiro == "" || txtTipoParceiro == 0) {
        erros += "<p>Informar o tipo do Parceiro</p>"
    ...
    if (Editar == 'EDITAR') {
        UrlParametro = url + "J1B1N_CFOP_Parceiro/Editar";
        pData = {
            pCFOP: txtCFOP,
            pTipoParceiro: txtTipoParceiro
        }
    }
    else {
        UrlParametro = url + "J1B1N_CFOP_Parceiro/Salvar";
        pData = {
            pCFOP: txtCFOP,
            pTipoParceiro: txtTipoParceiro
        }
...
function Editar(pEditar) {
    ...
    pCFOP = $("#hdfIdCFOP").val();
    pDescricao = $("#hdfTipoParceiro").val();
    $("#txtIdItem").attr('disabled', true);
    $("#txtCFOP").val(pCFOP);
    $("#txtTipoParceiro").val(pDescricao);
    $("#idItem").val(pEditar);
}
    ...
    $.SmartMessageBox({
        ...
        if (ButtonPressed === "Sim") {
            $.ajax({
                ...
                cache: false,
                type: 'GET',
                url: url + "J1B1N_CFOP_Parceiro/Excluir",
                dataType: 'json',
                data: {
                    pCFOP: pCFOP
                },
